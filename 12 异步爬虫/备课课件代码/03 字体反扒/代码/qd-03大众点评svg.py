...
if __name__ == '__main__':
    ...
    # 获取 svg 数据
    svg_url = re.findall(
        'background-image: url\((//s3plus.sankuai.com/v1/mss_0a06a471f9514fc79c981b5466f56b91/svgtextcss/.*?.svg)\);',
        css_response.text)
    # print(svg_url)
    svg_response = get_response('https:' + svg_url[0])
    print(svg_url)
    print(svg_response)
    ...
    if coord:
        ...
        coord_x, coord_y = int(float(coord_x)), int(float(coord_y))
    #
    ...
    print(coord_name, svg_text[position])
